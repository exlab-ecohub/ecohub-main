...
public class FeedbackAdminDtoModelAssembler implements RepresentationModelAssembler<FeedbackAdminDto, EntityModel<FeedbackAdminDto>> {
    ...
    public EntityModel<FeedbackAdminDto> toModel(FeedbackAdminDto entity) {
        return EntityModel.of(entity, linkTo(methodOn(AdminFeedbackController.class).getFeedbackById(entity.getId())).withSelfRel(), linkTo(methodOn(AdminFeedbackController.class).getAllMessages(entity.getResponseStatus().name(), entity.getMessageTopic().name())).withRel("messages"));
    }
